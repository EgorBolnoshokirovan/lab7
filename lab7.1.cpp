...
void init(structstack* stk) {
	stk->top = 0;
}

void push(structstack* stk, intk) {
	if (stk->top < N) {
		stk->elem[stk->top] = k;
		stk->top++;
	}
...
void DFSstack(structstack* stk, ints, int* G, int* num) {
	int i = 0;
	num[s] = 1;
	printf("%d ", s);
	push(stk, s);
...
void addElem(intdata, smej* head) {
	// Выделение памяти под корень списка
	smej* tmp = newsmej;
...
void DFSsp(smej* (&P)[N], intver, int* num) {
	num[ver] = 1;
	printf("%d ", ver);
	smej* s = P[ver];
	s = s->next;
